(() => {
  ...
  const navLinks = new Map(
    [...document.querySelectorAll('.lnav a')].map((a) => [a.getAttribute('href').slice(1), a]),
  );
    ...
    (es) => {
      for (const e of es)
        if (e.isIntersecting) {
          ...
          if (e.target.id !== activeH1) {
            activeH1 = e.target.id;
            buildRail(activeH1);
          }
        }
    },
  ...
  const subSpy = new IntersectionObserver(
    (es) => {
      for (const e of es)
        if (e.isIntersecting) {
  ...
  function renderResults(q) {
    ...
    for (const d of hits) {
      ...
      cx.className = 's-ctx';
      cx.textContent = d.text.slice(0, 90);
      a.append(ti, cx);
      results.append(a);
    }
  }
  ...
  document
    .querySelector('.logo')
    ?.addEventListener('click', () => document.getElementById('lnav')?.classList.toggle('open'));
})();
